...
(: 
 ...
 :   Generated at timestamp: 2016-04-19T21:47:42.919442-07:00
 :   Persisted by AUTHOR
 :   Date: DATE
 :)
module namespace imtitle = "http://refCombo#Imtitle-0.0.1";
...
) as map:map
{
    json:object()
        ...
        =>i:with($source-node/Customer/Address,      'Address',                    data($source-node/Customer/Address))
...
) as map:map
{
    json:object()
        ...
        =>i:with($source-node/Product/ProductID,     'ProductID',                   data($source-node/Product/ProductID))
...
) as map:map
{
    json:object()
        ...
        (: The following lines are generated from the Order entity type 
         ...
         : data to populate instances.
         :)
        =>i:with($source-node/Order/OrderID,         'OrderID',                 data($source-node/Order/OrderID))
        =>i:with($source-node/Order/CustomerID,      'CustomerID',                 
            if ($source-node/Order/CustomerID/element())
            then json:to-array($source-node/Order/CustomerID ! imtitle:extract-instance-Customer(.))
            else data($source-node/Order/CustomerID))
        =>i:with($source-node/Order/OrderDate,       'OrderDate',                 data($source-node/Order/OrderDate))
        =>i:with($source-node/Order/ShipAddress,     'ShipAddress',                 data($source-node/Order/ShipAddress))
        =>i:with($source-node/Order/OrderDetails,    'OrderDetails',                 $source-node/Order/OrderDetails/node())
...
) as map:map
{
    json:object()
        ...
        (: The following lines are generated from the OrderDetail entity type 
         ...
         : data to populate instances.
         :)
        =>i:with($source-node/OrderDetail/ProductID, 'ProductID',                       
            if ($source-node/OrderDetail/ProductID/element())
            then json:to-array($source-node/OrderDetail/ProductID ! imtitle:extract-instance-Product(.))
            else data($source-node/OrderDetail/ProductID))
        =>i:with($source-node/OrderDetail/UnitPrice, 'UnitPrice',                       data($source-node/OrderDetail/UnitPrice))
        =>i:with($source-node/OrderDetail/Quantity,  'Quantity',                       data($source-node/OrderDetail/Quantity))
