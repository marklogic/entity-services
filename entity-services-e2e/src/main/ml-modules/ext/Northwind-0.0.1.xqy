...
(: otherwise populate this instance :)
    ...
    let $cus := xs:string($source-node/Order/CustomerID)
	let $Doc := cts:search( collection("raw"), cts:element-value-query(xs:QName("CustomerID"), $cus))
    return json:object()
(: The following line identifies the type of this instance.  Do not change it.      :)
        =>map:with('$type', 'Order')
(: The following line adds the original source document as an attachment.           :)
...
(:                                                                                  :)
 ...
 =>es:optional('CustomerID',             xs:string($source-node/Order/CustomerID))
 ...
 (: The following property is a local reference.                                     :)
 =>es:optional('OrderDetails',           northwind:extract-array($source-node/Order/OrderDetails/*, function($x) {  json:object()=>map:with("$type", "OrderDetail")=>map:with("$ref", ($x)) }))
