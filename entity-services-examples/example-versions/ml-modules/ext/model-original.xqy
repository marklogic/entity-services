...
module namespace m = "http://marklogic.com/example/Model";
import module namespace es = "http://marklogic.com/entity-services"
    at "/MarkLogic/entity-services/entity-services.xqy";

declare function m:extract-instance-Person(
    $source as node()?
) as map:map
{
    let $source-node := m:init-source($source, 'Person')

    let $instance := json:object()=>m:add-attachments($source-node, $source)
...
(:~
 ...
 :)
declare function m:instance-to-canonical-xml(
...
return element { $key } { m:instance-to-canonical-xml($prop) }
...
m:instance-to-canonical-xml($val) }
else element { $key } {
attribute datatype { "array" },
$val }
...
(:
 ...
 :)
declare function m:instance-to-envelope(
$entity-instance as map:map
) as document-node()
{
document {
    element es:envelope {
        element es:instance {
            element es:info {
                ...
                element es:version { "original" }
            },
                m:instance-to-canonical-xml($entity-instance)
            },
...
declare private function m:init-source(
    $source as node()*,
...
declare private function m:add-attachments(
