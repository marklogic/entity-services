...
(:
 : This is a down-converting hub.
 : It has only next-version envelopes, but
 : responds to requests for the original model with a transform.
 :)
    ...
    let $result :=
        ...
        then
            ...
            then options:results($q, $options:hub-next,
                function($x) {
                    $x => translator:down-convert()
                    =>m-next:instance-to-envelope()
                    =>es:instance-json-from-document()
                })
            else options:results($q, $options:hub-next, es:instance-json-from-document#1)
        else
...
(: this hybrid hub doesn't accept new data.  It uses
 ...
 : Unlike hub-C, this one replaces the original data though.
 :)
...
) as document-node()
{
    ...
    return
        (
            xdmp:node-replace($envelope/es:envelope/es:instance, $next-envelope/es:envelope/es:instance),
            xdmp:document-add-collections(fn:base-uri($envelope), "instance-next-envelopes"),
            xdmp:document-remove-collections(fn:base-uri($envelope), "instance-envelopes")
        ),
    document {
        "OK"
    }
