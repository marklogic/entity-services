...
{es-codegen:comment("'" || $entity-type-name || "' entity type")}
...
(:                                                                                  :)
 {
    ...
    let $property-comment :=
        ...
        then es-codegen:comment("The following property is a local reference.")=>concat("&#10; ")
        else concat(
...
) as xs:string
{
    ...
    let $comment :=
        ...
        then es-codegen:comment("Warning: potential data loss, truncated array.")=>concat("&#10;")
        else ()
    let $function-call-string :=
        if ($is-required)
        then "=>   map:with("
...
(: defined in                                                                       :)
{es-codegen:comment($target-title || ", version " || $target-version)}
(: from documents that were persisted according to model                            :)
{es-codegen:comment($source-title || ", version " || $source-version)}
(:                                                                                  :)
(: Modification History:                                                            :)
{es-codegen:comment("Generated at timestamp: " || fn:current-dateTime())}
(:   Persisted by AUTHOR                                                            :)
